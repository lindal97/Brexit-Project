...
def hashed_string(string):
    hashed = str(hashlib.sha1(string.encode('utf-8')).hexdigest())
    return hashed
...
class MyListener(StreamListener):
    def on_status(self, status):
        ...
        if table is not None:
            ...
            hashed_id = hashed_string(status.user.id_str)
            user_info = (hashed_id,  
            ...
            c.execute("SELECT * FROM users WHERE hash_id = '" + str(hashed_id) + "'")
            re = c.fetchall()
            if len(re) == 0:
                try:
                    c.execute(''' INSERT INTO users
                                  (hash_id, user_description, 
                                  user_location, user_followers, user_friends, user_created_at)
                                  VALUES (%s,%s,%s,%s,%s,%s)''', user_info)
                except Exception as e:
                    logging.warning('Encountered error when scraping user info' +
                                        str(e))
                    pass
            ...
            if hasattr(status, 'retweeted_status'):
                ...
                retweet_uid = hashed_string(status.retweeted_status.user.id_str)
            ...
            if hasattr(status, 'quote_status'):
                ...
                retweet_uid = hashed_string(status.quoted_status.user.id_str)
                retweet=status.quoted_status.text
                retweet=remove_emoji(retweet)
            order = 'insert into '+ table + '''(id, hash_uid, 
                created_at, full_text, 
                hash_reply_id, 
                hash_retweet_uid, retweet_id, retweet)
                VALUES (%s,%s,%s,%s,%s,%s,%s,%s)'''
            if status.in_reply_to_user_id_str is not None:
                reply_uid = hashed_string(status.in_reply_to_user_id_str)
            try:
                twitter = (status.id_str, hashed_id, 
                       status.created_at, status.text,  
                       reply_uid, retweet_id, retweet_uid,retweet)
...
try:
    ...
    c = conn.cursor()
    c.execute('''CREATE TABLE if NOT EXISTS users(user_id CHAR(24), user_screen_name CHAR(32), username CHAR(32),
    ...
    while not twitter_stream.running:
        try:
        ...
        except (TimeoutError, SSLError, ReadTimeoutError, ConnectionError) as e:
            ...
            time.sleep(120)
    logging.critical('Somehow zombie has escaped.... (c\o tweepy github issue 617)')
except Exception as e:
    time.sleep(120)
